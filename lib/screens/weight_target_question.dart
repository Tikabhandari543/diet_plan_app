...
class WeightTargetScreen extends StatelessWidget {
  ...
  double _calculateCalories(double selectedWeight) {
    if (message == 'weight loss') {
      ...
      if (selectedWeight == 0.5) {
        return calorie - 500; // 0.5 kg weight loss requires 500 kcal deficit per day
      } else if (selectedWeight == 1.0) {
        return calorie - 1000; // 1 kg weight loss requires 1000 kcal deficit per day
      }
    } else {
      // Weight gain requires calorie surplus
      if (selectedWeight == 0.5) {
        return calorie + 500; // 0.5 kg weight gain requires 500 kcal surplus per day
      } else if (selectedWeight == 1.0) {
        return calorie + 1000; // 1 kg weight gain requires 1000 kcal surplus per day
      }
    }
  ...
  void _handleWeightSelection(BuildContext context, double selectedWeight) {
    ...
    Navigator.of(context).push(
      ...
      NutritionDisplay(calories: calculatedCalories, protein: protein, carbs: carbs, fat: fats)
      ),
    ); 
  }
